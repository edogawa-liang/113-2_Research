...
class TwoHopNodeAnalyzer:
    ...
    def analyze_degree_in_2hop(self, selected_nodes, changed, unchanged):
        ...
        for node in self.data.test_mask.nonzero(as_tuple=True)[0]:
        ...
        # Boxplot for 2-hop degrees (changed vs unchanged)
        ...
        plt.tight_layout()
        box_path = os.path.join(self.save_dir, f"{self.dataset}_{self.remain_model}_2hop_deg_box_by_group.png")
        ...
        stat_path = os.path.join(self.save_dir, f"{self.dataset}_{self.remain_model}_2hop_deg_stat.csv")
        stat_result.to_csv(stat_path, index=False)
        print(f"Saved statistical results to: {stat_path}")

        # Barplot for nearest/avg degree
        ...
        plot_binned_bar(nearest_degrees, "Binned Nearest Degree of Selected Nodes in 2-hop",
                        f"{self.dataset}_{self.remain_model}_nearest_deg_bar.png")

        plot_binned_bar(avg_degrees, "Binned Avg Degree of Selected Nodes in 2-hop",
                        f"{self.dataset}_{self.remain_model}_avg_deg_bar.png")

    def run(self):
        unchanged, changed = self.get_unchanged_nodes()
        selected_nodes = self.get_selected_nodes()
        self.analyze_degree_in_2hop(selected_nodes, changed, unchanged)
