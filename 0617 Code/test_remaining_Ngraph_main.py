...
def parse_args():
    ...
    parser.add_argument("--run_modes", nargs='+', required=True, help="List of explainer run_mode paths under saved/core_subgraph_mask/split_{split_id}/")
    parser.add_argument("--trial_numbers", nargs='+', type=int, required=True, help="List of corresponding trial numbers for each run_mode.")

    return parser.parse_args()


# python test_remaining_main.py \
#   --dataset Cora \
#   --split_id 0 \
#   --run_mode merged_test \
#   --run_modes run_a run_b run_c \
#   --trial_numbers 0 1 2
...
if __name__ == "__main__":
    ...
    assert len(args.run_modes) == len(args.trial_numbers), "Number of run_modes must match number of trial_numbers"

    print("\nLoading and combining core subgraph masks from multiple runs...")

    combined_edge_mask = None
    combined_feature_mask = None

    for run_mode, trial_num in zip(args.run_modes, args.trial_numbers):
        explain_dir = os.path.join("saved", "core_subgraph_mask", f"split_{args.split_id}", run_mode, args.dataset)

        print(f"→ Loading mask from {explain_dir} (trial {trial_num})")
        ...
        feat_mask, edge_mask = extractor.load()

        if combined_edge_mask is None:
            combined_edge_mask = edge_mask.copy()
            combined_feature_mask = feat_mask.copy()
        else:
            combined_edge_mask = np.logical_or(combined_edge_mask, edge_mask).astype(int)
            combined_feature_mask = np.logical_or(combined_feature_mask, feat_mask).astype(int)

    # 使用合併後的遮罩
    edge_removed_mask = combined_edge_mask
    feature_removed_mask = combined_feature_mask
